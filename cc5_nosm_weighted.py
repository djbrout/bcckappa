...
def convolve_maps(smoothing_grid,some_fit,some_map,filename,isFg):
    ...
    sigma = smoothing_grid#    print sigma
    if sigma > 0:
        if isFg:
...
def make_smooth_plots():
    ...
    plt.ylabel("$\kappa$ x Fg(weighted)")
    plt.ylim(0,1)
...
def analyze_ks_output(filename,eff_smoothing_grid,mag_cut,config_smoothing,pix,f):
    ...
    #smoothing_grid = eff_smoothing_grid
...
for mag in mag_cut_grid:
        ...
        print "Running Catalogue with Mag Cut: "+str(mag)
        ...
        for pix in pixel_scale_grid:
            ...
            run_ks_mapping()
            analyze_ks_output(filename,config_smoothing,mag,config_smoothing,pix,f)
